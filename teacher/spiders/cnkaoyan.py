...
class CnkiSpider(scrapy.Spider):
    name = 'cnkaoyan'
    start_urls = ['http://www.chinakaoyan.com/']
    xin=Xin()
    s_dic=SchoolDic()
    pageSchool={}
    school={}
    file= ''
    split = ["硕士研究生导师信息介绍：","硕士研究生指导教师介绍：","硕士研究生指导教师介绍:","硕士导师介绍：","博士生导师简介:","名师介绍：","教授简介：","研究生老师：","老师介绍：","研究生导师介绍：","研究生导师简介:","导师简介：","硕士生导师：","研究生导师：","博士生导师介绍：","博导介绍：","博士生导师：","导师信息：","硕导介绍：","硕导介绍－","导师介绍：","导师介绍——","导师介绍:","导师："]
    ...
    def parse(self, response):
        self.file=open('teacher/data/teacher2.csv','w',encoding='utf8')
        self.file.write("学校,学院,姓名,url\n")
        area=response.xpath('//div[@class="soso_area_box"]/a')
        for a in area:
            link=self.getValue(a.xpath("./@href"),None)
            url="http://www.chinakaoyan.com"+link
            yield scrapy.Request(url, callback=self.parseArea)

    def parseArea(self, response):
        school={}
        area = response.xpath('//div[@class="colu-info-body"]/div[@class="item"]/h3/a')
        for a in area:
            link = self.getValue(a.xpath("./@href"), None)
    ...
    def getLink(self,url):
        index=url.find("schoolID")
        id=url[index+9:]
        link="http://www.chinakaoyan.com/graduate/InfoList/class/ds/schoolID/" + id
        return link

    def parseLink(self, response):
        # print(response.url)
        teacher = response.xpath('//ul[@class="list-group font14"]/li/a')
        page= response.xpath('//div[@class="page font16"]/a')
        school=self.getValue(response.xpath("//div[@class='s-logo fl']/h1/a/text()"),None)
        # print("school:"+school)
        teacherList={}
        for s in teacher:
            link = "http://www.chinakaoyan.com"+self.getValue(s.xpath('./@href'),None)
        ...
        for t in teacherList:
            ...
            if isInst==False and len(nam) and len(r)<2:
                r.append(nam)
            if len(r)==2:
                self.file.write(school + "," + inst + "," + r[1] + ',' + teacherList[t] + '\n')

        if response.url.find('pagenum')<0:
    ...
    def getPageUrl(self,url,page):
        return url[0:-6]+"/pagenum/"+str(page)+'.shtml'

    def getMaxPage(self,page):
        list=page.xpath('./@href')
        if len(list)>0:
            url=list[-1].extract()
            page=url[url.find('pagenum/')+8:-6]
            return page
        return 0
