...
cntVector = CountVectorizer(stop_words=stpwrdlst,min_df=3)
cntTf = cntVector.fit_transform(corpus)
K=range(10,100)
meandistortions=[]
for k in K:
    print(str(k) + ':' + time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())))
    lda = LatentDirichletAllocation(n_topics=k,learning_offset=50.,random_state=0)
    docres = lda.fit_transform(cntTf)
    vau=lda.perplexity(cntTf)
    meandistortions.append(vau)
